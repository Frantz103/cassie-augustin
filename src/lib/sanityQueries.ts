...
export async function getHomepage(
  ...
  const client = getSanityClient();
  return client.fetch(groq`*[_type == "homepage" && language == $locale][0]`, {
    locale,
  });
}
...
export async function getAbout(
  ...
  const client = getSanityClient();
  return client.fetch(groq`*[_type == "about" && language == $locale][0]`, {
    locale,
  });
}
...
export async function getContact(
  locale: Locale = DEFAULT_LOCALE,
): Promise<SanityContact | null> {
  const client = getSanityClient();
  return client.fetch(groq`*[_type == "contact" && language == $locale][0]`, {
    locale,
  });
}
...
export async function getTestimonials(
  ...
  const client = getSanityClient();
  return client.fetch(
    groq`*[_type == "testimonial" && language == $locale] | order(order asc, _createdAt desc) [0...$limit] {
...
export async function getNavPages(
  locale: Locale = DEFAULT_LOCALE,
): Promise<Pick<SanityPage, "title" | "slug">[]> {
  const client = getSanityClient();
  return client.fetch(
