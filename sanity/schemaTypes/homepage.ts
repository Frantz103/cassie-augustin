...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'text',
      description:
        'Text below the call to action button (e.g., "Follow along for nutrition tips...")',
    }),
    defineField({
      name: 'heroImage',
      title: 'Hero Image',
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((value, context) => {
          const document = context.document as {heroImage?: unknown} | undefined
          if (document?.heroImage && !value) return 'Alt text is required when a hero image is set'
          return true
        }),
    }),
    defineField({
